...
@section('title', 'DESIGNER LIST PAGE')
...
<div class="alert w-75 mx-auto" style="margin-top: 30px;background-color:rgba(13, 78, 13, 0.651)">
  ...
  @endif
  <h1 class="text-center text-primary">Designer Data - {{ $designerData->total() }}</h1>
  {{-- if there is no  data --}}
  @if (count($designerData) == 0)
...
@else
 ...
 <table class="table">
    <thead>
      ...
      @foreach ($designerData as $designer)
      <tbody>
        <tr>
          <td scope="col-2">{{$designer->name}}</td>
          <td scope="col-3">
            <a href="{{route('edit.designer',$designer->id)}}" title="edit">
                <button class="btn btn-warning me-2">
                    <i class='fas fa-edit'></i>
                </button>
            </a>
              <a href="{{route('designer.detail',$designer->id)}}" title="Detail">
              <button class="btn btn-success me-2">
                  <i class='fas fa-folder-open'></i>
              </button>
          </a>
              <a href="{{route('designer.delete',$designer->id)}}" title="Delete">
